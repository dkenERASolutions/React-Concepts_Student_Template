...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "MySite" }) => {
    // Field values: useState (simple, independent inputs)
    ...
    // For every user we have, it is going to be recorded in Firestore
    ...
    // Derived validation (cheap, computed on every render)
    const isEmailValid = /\S+@\S+\.\S+/.test(email);
    const isPasswordValid = password.length >= 6;
    const isConfirmPasswordValid = confirmPassword.length >= 6;

    // UseMemo: memoized, mode-aware copy for the headings
    const copy = useMemo(() => {
        if (flow.mode === "login") {
            return {
                email: { title: "Welcome Back", subtitle: `Login in to your ${brandName} account` },
                password: { title: "Enter your passwords", subtitle: "Good to see you again!" },
    ...
    const handleSignOut = () => {
        ...
        // not wherever we happened to be in the signup flow
        setEmail("");
        setPassword("");
        setConfirmPassword("");
        setShowPassword(false);
    ...
    useEffect(() => {
        if (flow.status === "success") fireSideCannons();
    }, [flow.status]);
